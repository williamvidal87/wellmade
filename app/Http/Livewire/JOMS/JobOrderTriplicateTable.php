...
class JobOrderTriplicateTable extends Component
{
    ...
    {
        $this->dispatchBrowserEvent('swal:confirmUnlockAccess', [
    ...
    public function export2PDF($dataID){
        $job_order_data = JobOrder::with('clientProfile.contacts', 'clientProfile.forCSA', 'getContact')->where('id',$dataID)->whereIn('status', [1,2,3,4,9])->whereIn('payment_status_id', [12,13])->first();
        $latestID = JobOrder::where('status',9)->where('payment_status_id', 12)->count();
        $related_work_order = WorkOrder::with('getERScope','getScopeDescription', 'getDiscount', 'getGeneralProcedure')->where('jo_no_id',$job_order_data->id)->whereIn('status', [5,6,9])->get();
        $cancelled_work_order = WorkOrder::with('getJobOrder.getContact', 'getCancelReason', 'getUserCancel')->where('jo_no_id',$job_order_data->id)->where('status', 3)->get();
        ...
        if (count($related_work_order) > 0) {
            ...
            $exist = [];
            foreach ($related_work_order as $key => $value) {
    ...
    {
        return view('livewire.j-o-m-s.job-order-triplicate-table', [
            // 'job_orders_triplicate' => JobOrder::with('clientProfile', 'getStatus')->where('status', 9)->where('payment_status_id', 12)->get(),
            // 'job_orders_triplicate' => TransactionSummary::where('receipt_no', 'LIKE', '%WV%')->with('jobOrder.clientProfile', 'jobOrder.getStatus')->where('transaction_status_id', 2)->where('status_id', 12)->get(),
            'job_orders_triplicate' => TransactionSummary::where('wv_invoice_no', 'LIKE', '%WV%')
